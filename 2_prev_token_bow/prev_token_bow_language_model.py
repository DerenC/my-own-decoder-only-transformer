...
class PrevTokenBOWLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # Cumulative sum (prep for uniform BOW up to each position)
        cumsum = token_embs.cumsum(dim=1)   # Shape: (B, T, n_emb)

        # Divide by the number of tokens so far to get the means
        num_of_tokens = torch.arange(1, T+1).view(1, T, 1)
        bow = cumsum / num_of_tokens    # Shape: (B, T, n_emb)

        logits = self.lang_modelling_head(bow)  # Shape: (B, T, vocab_size)
    ...
    def generate(self, idx, num_max_new_tokens):
        ...
        for _ in range(num_max_new_tokens):
            # Get the predictions
            logits, __ = self(idx)  # loss is ignored during inference
