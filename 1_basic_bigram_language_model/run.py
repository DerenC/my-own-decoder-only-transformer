...
## RANDOM MODEL PREDICION & INFERENCE
first_token = torch.zeros((1,1), dtype=torch.long)    # In our case, 0 is the newline character

assert first_token < vocab_size

rand_generated_seq = decode(model.generate(first_token, num_max_new_tokens=100)[0].tolist())
print(f'Initial "random" loss: {loss}')
print(rand_generated_seq)
...
generated_seq_aft_training = decode(model.generate(first_token, num_max_new_tokens=1000)[0].tolist())
